...
export default function MusicPage() {
    ...
    const handleExtractAndPlay = async (videoId: string) => {
      try {
        //const mergedUrl = `http://localhost:8080/api/video/merge?videoUrl=${encodeURIComponent(videoUrl)}&audioUrl=${encodeURIComponent(audioUrl)}`;
        ...
        console.error("스트림 추출 실패", err);
        alert("유효한 유튜브 URL인지 확인해주세요.");
      }
    };
    ...
    return (
      <div>
        ...
        {videoSrc && (
          <video controls autoPlay className="mt-6 w-full max-w-4xl rounded shadow">
            <source src={videoSrc} type="video/mp4" />
            브라우저가 video/mp4 포맷을 지원하지 않습니다.
          </video>
        )}
